        ...
        <AnimatePresence>
          {isHovered && (
            <motion.div
              ...
              className={`fixed -translate-x-1/2 w-max min-w-[220px] bg-[#0a0a0a]/95 backdrop-blur-2xl border border-white/10 rounded-xl p-4 z-[999999] shadow-[0_20px_50px_-10px_rgba(0,0,0,0.8)] font-sans text-sm text-left ${coords.showAbove ? 'bottom-auto' : 'top-auto'}`}
              style={{ 
                left: coords.left, 
            ...
            >
              <div className="text-white/70 mb-3 font-normal tracking-wider text-xs uppercase border-b border-white/5 pb-2">
                <span className="text-secondary font-medium">interface</span> <span className="text-white/90">{keyword}</span> {'{'}
              </div>
              {definition.map((def, i) => (
                <div key={i} className="ml-2 my-1.5 flex items-center gap-2">
                  ...
                  <span className={`font-sans ${typeof def.value === 'string' ? 'text-secondary opacity-90' : 'text-white'}`}>
