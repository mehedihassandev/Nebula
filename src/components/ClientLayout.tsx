...
export const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent | TouchEvent) => {
      const isActivityBarBtn = (event.target as Element).closest('.activity-bar-btn');
      if (popupRef.current && !popupRef.current.contains(event.target as Node) && !isActivityBarBtn) {
        setActivePopup(null);
      }
    };
  ...
  useEffect(() => {
    const handleGlobalShortcuts = (e: KeyboardEvent) => {
      ...
      if (cmdOrCtrl && e.shiftKey && (e.key.toLowerCase() === 'e' || e.code === 'KeyE')) {
        e.preventDefault();
        e.stopPropagation();
        setActiveTab('explorer');
        setActivePopup(null);
      }
      if (cmdOrCtrl && e.shiftKey && (e.key.toLowerCase() === 'f' || e.code === 'KeyF')) {
        e.preventDefault();
        e.stopPropagation();
        setActiveTab('search');
        setActivePopup(null);
      }
      if (cmdOrCtrl && e.shiftKey && (e.key.toLowerCase() === 'x' || e.code === 'KeyX')) {
        e.preventDefault();
        e.stopPropagation();
  ...
  return (
    <div data-theme={theme} className="flex flex-col h-screen w-screen bg-primary text-textColor overflow-hidden font-sans relative">
        ...
        <div className="relative flex z-[100]">
          ...
          <AnimatePresence>
            {activePopup && (
              ...
              >
                {activePopup === 'account' ? <AccountPanel /> : <SettingsPanel />}
              </motion.div>
            )}
          </AnimatePresence>
        </div>
